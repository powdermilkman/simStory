...
Use the provided update script to automatically pull the latest code from GitHub and update your installation:
...
This script will:
- Rebuild the container with the latest code from GitHub
- Run database migrations
...
2. Rebuild with latest code (--no-cache forces a fresh clone from GitHub):
```bash
...
This will show you the latest commits from GitHub. Note: This only works if the container has git history.
