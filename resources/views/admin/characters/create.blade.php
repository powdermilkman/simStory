<x-admin-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Create Character</h2>
    </x-slot>
        ...
        <form action="{{ route('admin.characters.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-2 gap-4 mb-4">
                ...
                <div>
                    ...
                    <input type="datetime-local" name="fake_join_date" id="fake_join_date" value="{{ old('fake_join_date') }}"
                        class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
            ...
            <div class="mb-4">
                ...
                <textarea name="signature" id="signature" rows="2" maxlength="500"
                    class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">{{ old('signature') }}</textarea>
                @error('signature')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-6 p-4 bg-gray-50 rounded-lg">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <input type="range" name="bytes" id="bytes" min="0" max="5" value="{{ old('bytes', 0) }}"
                        class="w-32" oninput="document.getElementById('bytes-display').textContent = this.value">
                    <span id="bytes-display" class="text-lg font-medium text-gray-900">{{ old('bytes', 0) }}</span>
