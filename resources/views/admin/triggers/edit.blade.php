<x-admin-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Edit Trigger: {{ $trigger->name }}</h2>
    </x-slot>

    <div class="bg-white rounded-lg shadow p-6 max-w-4xl">
        <form action="{{ route('admin.triggers.update', $trigger) }}" method="POST" x-data="triggerForm()">
            @csrf
            @method('PUT')

            <div class="mb-4">
                <label for="name" class="block text-sm font-medium text-gray-700 mb-1">Trigger Name</label>
                <input type="text" name="name" id="name" value="{{ old('name', $trigger->name) }}" required
            ...
            <div class="mb-4">
                ...
                <input type="text" name="identifier" id="identifier" value="{{ old('identifier', $trigger->identifier) }}" required
            ...
            <div class="mb-4">
                ...
                <textarea name="description" id="description" rows="2"
                    class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">{{ old('description', $trigger->getRawOriginal('description')) }}</textarea>
            </div>
            ...
            <div class="mb-6">
                ...
                <template x-for="(condition, index) in conditions" :key="index">
                    <div class="mb-3 p-4 bg-gray-50 rounded-lg border">
                        <input type="hidden" :name="'conditions[' + index + '][id]'" x-model="condition.id">
            ...
            <div class="flex items-center gap-4">
                ...
                <a href="{{ route('admin.triggers.show', $trigger) }}" class="text-gray-600 hover:text-gray-900">Cancel</a>
            </div>
        </form>
    </div>

    @php
        $conditionsData = $trigger->conditions->map(function($c) {
            return [
                'id' => $c->id,
                'type' => $c->type,
                'target_id' => $c->target_id,
                'choice_option_id' => $c->choice_option_id,
            ];
        })->values()->toArray();
        if (empty($conditionsData)) {
            $conditionsData = [['type' => '', 'target_id' => '', 'choice_option_id' => '']];
        }
    ...
    <script>
        function triggerForm() {
            return {
                ...
                addCondition() {
                    this.conditions.push({ id: null, type: '', target_id: '', choice_option_id: '' });
                },
