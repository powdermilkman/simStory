<x-admin-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Edit Thread: {{ $thread->title }}</h2>
    </x-slot>
        ...
        <form action="{{ route('admin.threads.update', $thread) }}" method="POST">
                ...
                <div>
                    ...
                    <select name="author_id" id="author_id" required
                        ...
                        @foreach($characters as $character)
                            <option value="{{ $character->id }}" {{ old('author_id', $thread->author_id) == $character->id ? 'selected' : '' }}>
            ...
            <div class="mb-4">
                ...
                <input type="datetime-local" name="fake_created_at" id="fake_created_at" 
                    value="{{ old('fake_created_at', $thread->fake_created_at?->format('Y-m-d\TH:i')) }}"
                    class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
            ...
            <div class="grid grid-cols-2 gap-4 mb-4">
                <div class="flex items-center">
                    <input type="checkbox" name="is_pinned" id="is_pinned" value="1" {{ old('is_pinned', $thread->is_pinned) ? 'checked' : '' }}
                        class="rounded border-gray-300 text-blue-600 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                    <label for="is_pinned" class="ml-2 text-sm text-gray-700">Pin this thread</label>
                </div>

                <div class="flex items-center">
                    <input type="checkbox" name="is_locked" id="is_locked" value="1" {{ old('is_locked', $thread->is_locked) ? 'checked' : '' }}
                        class="rounded border-gray-300 text-blue-600 shadow-sm focus:border-blue-500 focus:ring-blue-500">
            ...
            <div class="mb-6">
                <x-searchable-select
                    ...
                    :value="old('phase_id', $thread->phase_id)"
                    display="name"
                    subtitle="identifier"
            ...
            <div class="flex items-center gap-4">
                ...
                <a href="{{ route('admin.threads.show', $thread) }}" class="text-gray-600 hover:text-gray-900">Cancel</a>
            </div>
        </form>
    </div>
</x-admin-layout>
